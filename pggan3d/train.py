...
def setup_snapshot_3d(G, training_set):
    ...
    labels = np.zeros([num_fakes, training_set.label_size], dtype=training_set.label_dtype)
...
def train_progressive_gan(
    ...
    grid_reals, grid_labels, grid_latents = setup_snapshot_3d(G, training_set)
    ...
    misc.save_mri_image(grid_reals, os.path.join(result_subdir, 'reals.nii.gz'), drange=training_set.dynamic_range)
    misc.save_mri_image(grid_fakes, os.path.join(result_subdir, 'fakes%06d.nii.gz'%0), drange=drange_net)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        prev_lod = sched.lod
        # tf.get_default_session().graph.finalize()

        # Run training ops.
        for repeat in range(minibatch_repeats):
            count+=sched.minibatch
            for _ in range(D_repeats):
                _,_,d_loss,g_loss = tfutil.run([D_train_op, Gs_update_op, D_loss, G_loss], {lod_in: sched.lod, lrate_in: sched.D_lrate, minibatch_in: sched.minibatch})
                cur_nimg += sched.minibatch
            tfutil.run([G_train_op], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch})

        d_loss_total+=np.sum(np.array(d_loss))/len(d_loss)
        g_loss_total+=np.sum(np.array(g_loss))/len(g_loss)

        # print(g_loss)

        # Perform maintenance tasks once per tick.
        # if  count > (sched.tick_kimg * 1000/2):
        #     print('Saving at {}'.format(cur_nimg))
        #     misc.save_pkl((G, D, Gs), os.path.join(result_subdir, 'network-snapshot-temp.pkl' ))
        #     count=0

        done = (cur_nimg >= total_kimg * 1000)
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
        # if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1 or done:
            cur_tick += 1
            cur_time = time.time()
            ...
            # Report progress.
            print('tick %-5d kimg %-8.1f g loss %-2.5f d loss %-2.5f lod %-5.2f minibatch %-4d time %-12s sec/tick %-7.1f sec/kimg %-7.2f maintenance %.1f' % (
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                ...
                grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=None)
                misc.save_mri_image(grid_fakes, os.path.join(result_subdir, 'fakes%06d.nii.gz'% (cur_nimg // 1000)))
