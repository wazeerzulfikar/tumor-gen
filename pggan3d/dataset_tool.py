    ...
    def add_image(self, img):
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            ...
            ex = tf.train.Example(features=tf.train.Features(feature={
                ...
                'data': tf.train.Feature(bytes_list=tf.train.BytesList(value=[quant.ravel().tostring()]))}))
...
def create_from_mri(tfrecord_dir, image_dir, shuffle):
    print('Loading mri images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*.nii.gz')))
    if len(image_filenames) == 0:
        error('No input images found')
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames))
        for idx in range(order.size):
            f = image_filenames[order[idx]]
            img = nib.load(f)
            img = nib.as_closest_canonical(img).get_fdata().clip(0,255)
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_from_mri', 'Create dataset from a directory full of images.',
                                            'create_from_images datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
